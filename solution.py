...
class SOLUTION:
    ...
    def __init__(self, myID, runName, runNum):
        self.myID = myID
    ...
    def Start_Simulation(self, directOrGUI):
        ...
        self.Create_Body()
        self.Create_Brain()
        while not os.path.exists("run{}/brain{}.nndf".format(self.runNum, self.myID)):
            time.sleep(0.1)
        not_finished = True

        while not_finished:
            with open("run{}/brain{}.nndf".format(self.runNum, self.myID), "r") as file:
                last_line_b = file.readlines()[-1]
            with open("run{}/world{}.sdf".format(self.runNum, self.myID), "r") as file:
        ...
        os.system("python simulate.py {} {} {} {} {}&".format(directOrGUI, self.myID, self.runName, self.runNum, self.startTime))

    def Wait_For_Simulation_To_End(self):
        while not os.path.exists("run{}/fitness{}.txt".format(self.runNum, self.myID)):
            time.sleep(0.01)
        not_finished = True 
        while not_finished:
            try:
                f = open("run{}/fitness{}.txt".format(self.runNum, self.myID), "r")
                fitnesses = f.read().split(',')
                fitnesses = [float(x) for x in fitnesses]
                self.fitness1 = fitnesses[0]
                self.fitness2 = fitnesses[1]
                f.close()
                not_finished = False
            except: 
                time.sleep(0.01)

        os.system("rm run{}/fitness{}.txt".format(self.runNum, self.myID))
    ...
    def Create_World(self):
        ...
        y = 0
        z = 0.5
        pyrosim.Start_SDF("run{}/world{}.sdf".format(self.runNum, self.myID))
        pyrosim.End()
    
    def Create_Block(self):
        pyrosim.Start_URDF("run{}/block.urdf".format(self.runNum))
        pyrosim.Send_Cube(name="Box", pos=[2.5,0,0.5], size=[0.5, 0.5, 0.5])
        pyrosim.End() 
    ...
    def Create_Body(self):
        ...
        pyrosim.Start_URDF("run{}/body.urdf".format(self.runNum))
    ...
    def Create_Brain(self):
        pyrosim.Start_NeuralNetwork("run{}/brain{}.nndf".format(self.runNum, self.myID))
        ...
        pyrosim.Send_Proximity_Neuron(name=4)
        

        pyrosim.Send_Motor_Neuron(name=5, jointName=b'Torso_BackLeg')
        pyrosim.Send_Motor_Neuron(name=6, jointName=b'Torso_FrontLeg')
        pyrosim.Send_Motor_Neuron(name=7, jointName=b'Torso_LeftLeg')
        pyrosim.Send_Motor_Neuron(name=8, jointName=b'Torso_RightLeg')
        pyrosim.Send_Motor_Neuron(name=9, jointName=b'LeftLeg_LeftLower')
        pyrosim.Send_Motor_Neuron(name=10, jointName=b'RightLeg_RightLower')
        pyrosim.Send_Motor_Neuron(name=11, jointName=b'BackLeg_BackLower')
        pyrosim.Send_Motor_Neuron(name=12, jointName=b'FrontLeg_FrontLower')
