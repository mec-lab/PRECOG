...
class NEURAL_NETWORK: 
    ...
    def Update(self, click):
        for k, v in self.neurons.items():
            ...
            elif self.neurons[k].Is_Proximity_Neuron():
                self.neurons[k].Update_Proximity_Neuron(click)
...
# ---------------- Private methods --------------------------------------
    ...
    def Print_Proximity_Neuron_Values(self):

        print("proximity neuron values: " , end = "" )

        for neuronName in sorted(self.neurons):

            if self.neurons[neuronName].Is_Proximity_Neuron():
