...
def Send_Proximity_Neuron(name):

    f.write('    <neuron name = "' + str(name) + '" type = "proximity" />\n')
...
def Send_Cube(name="default",pos=[0,0,0],size=[1,1,1], mass = 1.0):
    ...
    Send_Link(name,pos,size,"box", 1.0)
...
def Send_Motor_Neuron(name,jointName):
    ...
    f.write('    <neuron name = "' + str(name) + '" type = "motor"  jointName = "' + str(jointName) + '" />\n')

def Send_Sensor_Neuron(name,linkName):

    f.write('    <neuron name = "' + str(name) + '" type = "sensor" linkName = "' + str(linkName) + '" />\n')
